...
def get_reward_test(x_train, y_train, x_test, y_test, hp_setting):
 ...
 #   net = tl.layers.DropoutLayer(net, keep=drop_out_rate1, name='drop1_1')
    ...
    tl.utils.fit(sess, out, train_op, cross_entropy, x_train, y_train, x, y_,
                acc=accuracy, batch_size=200, n_epoch=2, print_freq=1,
...
np.savetxt('pop_reserve_sl_conv_mnist.txt',pop_reserve,delimiter=',')    
np.savetxt('fitness_reserve_sl_conv_mnist.txt',fitness_reserve,delimiter=',') 
...
np.save('test_error_sl_conv_mnist.npy',test_error) 
