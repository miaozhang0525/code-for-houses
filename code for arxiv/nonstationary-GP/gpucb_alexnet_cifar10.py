...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  3 21:28:06 2018
...
def get_reward_test(x_train, y_train, x_test, y_test, hp_setting):     
    ...
    for it in range(n_val):     
        ...
        tl.utils.fit(sess, out, train_op, cross_entropy, x_train, y_train, x, y_,
                    acc=accuracy, batch_size=50, n_epoch=200, print_freq=10,
...
def surragate_fitness(x,pop,fitness,w):   
    ...
    gp=GaussianProcessRegressor(
            kernel=nonstationaryRBF(length_scale=np.ones((pop.shape[1],)),length_scale_bounds=(1e-5,10),indx=tt),alpha=0.08)      
    X=pop
    Y=fitness    
    gp.fit(X,Y)   
...
np.savetxt('pop_reserve_alexnet_PI_cifar10.txt',pop_reserve,delimiter=',')    
np.savetxt('fitness_reserve_alexnet_PI_cifar10.txt',fitness_reserve,delimiter=',') 
...
np.save('select_hp_alexnet_PI_cifar10.txt',select_hp) 
np.save('test_error_alexnet_PI_cifar10.npy',test_error) 
